...
插件会将此 profile 的工作区目录选择器切换为 DSH 自带的网页目录浏览器。本机和远程网页均可选择宿主电脑上的目录，无需在宿主屏幕上操作系统弹窗；卸载插件后恢复默认选择器。

## 使用

### 配置模型

首次打开 DSH 时，按提示填入自己的 DeepSeek API Key，也可以稍后在 **设置 → 模型** 中配置。AgentLink 只提供远程连接，不提供模型密钥；未配置可用模型时，可以浏览工作区，但不能完成真实模型对话。
...
- 已从 GitHub 全新安装真实 DSH，并验证线上账号绑定、原生页面、远程目录选择、工作区事件同步、重启恢复、解绑和手机宽度布局。真实模型对话、工具审批、浏览器文件选择器收发和第三方插件尚未完成全面验收。
